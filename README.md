...
### Пуш уведомления
Пуш уведомления запускаются в компоузе.

### Локализация
Для локализации использовал библиотеку i18next.
1. как приложение узнает, какой язык выбран у пользователя? - С помощью плагина i18next-browser-languagedetector
2. акие типы контента поддерживают переключение языка и как вы это реализовали - Ссылка в футере отвечает за переключение языка, использовал методы   changeLanguage
3. как переводы попадают на клиент? как изменилась сборка приложения? - Пока что переводы попадают на клиент через обычные импорты, т.к. не получилось сделать подгрузку только нужных шрифтов с сервера. Сборка приложения никак не изменилась.
