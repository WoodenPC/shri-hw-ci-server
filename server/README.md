...
Базовый образ взял node:alpine, подтягивает _node js 13_ версии
...
`{ "repoName": "WoodenPC/shri-async-hw", "buildCommand": "npm run build", "mainBranch": "master", "period": 0 }`
...
- _axios_ - для работы с http запросами
- _nodemon_ - чтобы перезапускать сервер, при изменении файлов
- _express_ - фреймворк для ноды
- _date-fns_ - либа для работы с датами, по итогу можно было обойтись без нее, но она достаточно легковесная так что решил оставить
